...
#Generate csv files from ODS
	mkdir -p $(OUTPUT)
nounods:
	/Applications/LibreOffice.app/Contents/MacOS/soffice --headless --convert-to "csv:Text - txt - csv (StarCalc):9,34,UTF8" --outdir $(DATA_DIR)/nouns/ $(DATA_DIR)/nouns/*.ods
verbods:
	/Applications/LibreOffice.app/Contents/MacOS/soffice --headless --convert-to "csv:Text - txt - csv (StarCalc):9,34,UTF8" --outdir $(DATA_DIR)/verbs/ $(DATA_DIR)/verbs/*.ods
#Package files
...
verbsql:
	python3 $(SCRIPT)/verbs/gen_verb_dict_format.py -o sql  -v $(VERSION)  -f $(OUTPUT)/verbs.aya.dic > $(OUTPUT)/verbs.sql
...
nounsql:
	...
	# fa3il file
	python3 $(SCRIPT)/nouns/gen_noun_dict.py  -f $(DATA_DIR)/nouns/fa3il.csv  -v $(VERSION) -d sql -t fa3il  >$(OUTPUT)/nouns.dict.sql
	## maf3oul file
...
cust_ods:
	/Applications/LibreOffice.app/Contents/MacOS/soffice --headless --convert-to "csv:Text - txt - csv (StarCalc):9,34,UTF8" --outdir $(DATA_DIR)/custom/ $(DATA_DIR)/custom/*.ods
